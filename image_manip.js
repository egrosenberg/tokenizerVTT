...
const NOT_IMG_TYPE =
  "ERROR: Provided image data is not requisite image data structure";
...
const MID_USER_SCALE = 1.0;
const GRADIENT_SCALE = 0.7;
...
const blank_img_path = "img/blank.webp";
...
const clear_img_id = "clear_image";
const token_scale_id = "token_scale";
...
async function tokenize() {
  ...
  const gradientCanvas = createGradient(
    img_s,
    img_s,
    col1,
    col2,
  ...
  clipOnto(
    token_art_context,
    tempPeekContext,
    tempPeekCanvas.width,
    tempPeekCanvas.height
  );
  ...
  const promise = new Promise((resolve) => {
    tmpImg.onload = function () {
      displayCtx.drawImage(
        ...
        offsetX,
        offsetY,
        art_width * scale * relative_scale,
        art_height * scale * relative_scale
      );
      resolve();
    };
...
async function displayPreview() {
  ...
  let ringCan;
  if (!document.getElementById(ring_count_id).checked) {
    ringCan = !document.getElementById(border_style_id).checked
      ? await canvasFromPath(single_ring_overlay_path)
      : await canvasFromPath(single_square_overlay_path);
  } else {
    ringCan = !document.getElementById(border_style_id).checked
      ? await canvasFromPath(double_ring_overlay_path)
      : await canvasFromPath(double_square_overlay_path);
  }
...
async function instantiateImage(path = instructions_path) {
...
async function saveImage() {
  ...
  const { resizeW, resizeH, newX, newY } = getScaledVars(resized_canvas);

  let resizedImg = new Image();
  const resizePromise = new Promise((resolve) => {
    resizedImg.onload = function () {
      resized_context.clearRect(
        0,
        0,
        resized_canvas.width,
        resized_canvas.height
      );
      resized_context.drawImage(resizedImg, newX, newY, resizeW, resizeH);
      resolve();
    };
...
async function wbUndo() {
  ...
  redoCtx.putImageData(
    peek_context.getImageData(0, 0, peek_canvas.width, peek_canvas.height),
    0,
    0
  );
  ...
  const imgData = recovCtx.getImageData(
    0,
    0,
    peek_canvas.width,
    peek_canvas.height
...
$(document).ready(function () {
  ...
  $("#" + scale_id).change(function () {
    art_user_scale =
      (this.value / 100) * ((MID_USER_SCALE - MIN_USER_SCALE) / 0.5) +
      MIN_USER_SCALE;
    displayPreview();
  });
  // handle setting scale from slider
  $("#" + pan_h_id).change(function () {
    art_offset_user_x =
      (this.value - 50) * art_width * art_base_scale * rel_scale * 0.02;
    displayPreview();
  });
  // handle setting scale from slider
  $("#" + pan_v_id).change(function () {
    art_offset_user_y =
      (100 - this.value - 50) * art_height * art_base_scale * rel_scale * 0.02;
    displayPreview();
  });
  // number of rings on token (1/2)
  $("#" + ring_count_id).change(function () {
    displayPreview();
  });
  // toggle border shape (square/circle)
  $("#" + border_style_id).change(function () {
    displayPreview();
  });
  ...
  $("#" + brush_size_id).change(function () {
    ...
    brush_size =
      (this.value / 100) * (MAX_BRUSH_SIZE - MIN_BRUSH_SIZE) + MIN_BRUSH_SIZE;
  ...
  $(`#${clear_img_id}`).click(() => instantiateImage(blank_img_path));
  ...
  if (whiteboard) {
    ...
    $(board_frame)
      .mousedown(function (e) {
        ...
        let x =
          (canvasX - (art_offset_user_x + art_offset_x) * rel_scale) * scale;
        let y =
          (canvasY - (art_offset_user_y + art_offset_y) * rel_scale) * scale;
      ...
      .mousemove(function (e) {
        // move cursor
        document.getElementById("circular-cursor").style.left = `${
          e.pageX - brush_size
        }px`;
        document.getElementById("circular-cursor").style.top = `${
          e.pageY - brush_size
        }px`;
        if (isDown !== false) {
          // calculate in-canvas position
